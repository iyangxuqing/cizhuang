...
let methods = {
  ...
  onDescBlur: function (e) {
    let page = getCurrentPages().pop()
    let index = e.currentTarget.dataset.index
    let items = page.data.swiperItemsEditor.items
    items[index].desc = desc
    page.setData({
      'swiperItemsEditor.items': items
    })
    this.onItemsChanged && this.onItemsChanged(items)
  }
...
export class SwiperImagesEditor {
  ...
  constructor(options) {
    ...
    this.onItemsChanged = options.onItemsChanged || null
